...
C
     ...
     * threehalf = 1.5d0, safety = 1.d-20, tolSgn = 1.d-20, tolS12=1.d-1)              
C     
     ...
     *   C41 , C42 ,C43 ,C44
       ...
       double precision sgn_d_strain,A,B,s12_max,flow
...
C		  
				  ...
				  sgn_s12 = Sign(one, stressNew(i,4))
				  sgn_s12_old = Sign(one, stressOld(i,4))			 
				  
				  IF (flow.EQ.one) THEN                              ! flow=1: loading; flow=-1: unloading
					  
					  IF ((stateNew(i,7) .NE. stateOld(i,7)) .AND.
     *                          (sgn_s12 .NE. sgn_s12_old) ) THEN         ! Record the shear strain at the point that flow changes
						  stateNew(i,8) = stateOld(i,4)              ! SDV8: Origin of the nonlinear part/loading part
						  stateOld(i,9) = stateOld(i,4)
					  ENDIF
					  
					  IF ( sgn_d_strain*stateNew(i,4) .GE. 
     *					   sgn_d_strain*stateNew(i,9) ) THEN  ! SDV9: max strain experienced
						  
						  stateNew(i,9) = stateNew(i,4)
						  stressNew(i,4) = sgn_d_strain*abs(A*(one-
     *					     exp(-B*abs(stateNew(i,4)-stateNew(i,8)))))
	 
						  s12_max = stressNew(i,4) ! Record the s12_max using the way record the D
						  stateNew(i,10) = s12_max
						  
					  ELSE
					  
					      stateNew(i,9) = stateOld(i,9)
						  stressNew(i,4) = sgn_d_strain*abs(
     *					   abs(G12*(abs(stateNew(i,9)- stateNew(i,4))))
     *	                  -abs(s12_max))
					  
					  ENDIF
					  		  
				  ELSE IF (flow.NE.one) THEN
					  ...
					  stressNew(i,4) = sgn_d_strain*(
     *					   abs(G12*(abs(stateNew(i,5)- stateNew(i,4))))
     *	                  -abs(s12_max))					
